...
Send an [API key](/id/docs/general-concepts/api-keys-and-access) scoped for Sebar in the `X-API-Key` header. Sends are a server-side operation — use a secret key and keep it off the browser.
...
For task-oriented examples, see [How to send email](/id/docs/sebar/how-to-send-email).
