...
<Callout type="warn">
Add your CLI token as a repository **secret** named `KODENA_API_TOKEN` (GitHub → Settings → Secrets and variables → Actions). Mint a `koda_…` token in the Sawala dashboard under **Organization Settings → CLI tokens** — see [CLI tokens](/en/docs/general-concepts/api-keys-and-access#cli-tokens). Never commit a token.
</Callout>
...
<Callout type="info">
Want a plain `git push` to deploy too? Keep the optional `push:` trigger in the workflow above — it's independent of the dashboard button. And the [`kodena` CLI](/en/docs/kodena/reference/cli) still works for local or CI deploys; none of these are mutually exclusive.
</Callout>
...
See the [CLI reference](/en/docs/kodena/reference/cli) for manual deploys and [What is Kodena](/en/docs/kodena/what-is-kodena) for the bigger picture.
