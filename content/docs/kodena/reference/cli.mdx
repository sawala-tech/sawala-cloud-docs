...
These make the CLI usable in CI: set `KODENA_API_TOKEN`, `KODENA_ORG`, and `KODENA_PROJECT`, then run `kodena deploy --build` with no interactive login. See [How to deploy from GitHub](/id/docs/kodena/how-to-deploy-from-github) for a complete workflow.

For the authentication model behind `koda_…` tokens, see [API keys & access](/id/docs/general-concepts/api-keys-and-access#cli-tokens).
