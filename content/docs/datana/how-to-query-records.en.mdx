...
Datana runs queries **server-side**, so you ask for exactly the slice you need instead of fetching everything and filtering in the browser. Each recipe below shows the [`@sawala/datana-client`](/en/docs/general-concepts/cli-and-sdk) call and the equivalent raw request. All reads return only **published** records of **public** collections, with private fields stripped.
...
See the [API reference](/en/docs/datana/reference/api) for every parameter and the full response shape.
