...
All three modes require a project-scoped publishable key (`pk_live_…`) for the `formulir` product. See [API keys and access](/id/docs/general-concepts/api-keys-and-access).
...
Spam-protection errors surface as `CAPTCHA_REQUIRED`, `CAPTCHA_FAILED`, or `CAPTCHA_NOT_CONFIGURED`. See the [API reference](/id/docs/formulir/reference/api#error-codes).
