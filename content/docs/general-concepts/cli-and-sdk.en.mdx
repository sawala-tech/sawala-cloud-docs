...
Beyond the dashboard and the raw [REST APIs](/en/docs/general-concepts/api-keys-and-access), Sawala publishes ready-made tools so you don't have to hand-roll HTTP calls. There are three families, for three audiences:
...
SDKs authenticate with an **[API key](/en/docs/general-concepts/api-keys-and-access)** — typically a public key (`pk_live_…`) for read access. Each package's own README carries its quickstart and API reference. More packages ship over time.
...
Prefer to paste a token — or on a headless machine or CI? That flow is still supported: pass `--no-browser` to paste a token you minted under **Organization Settings → CLI tokens**, or `--token <koda_…>` to sign in non-interactively. See [API keys & access](/en/docs/general-concepts/api-keys-and-access#cli-tokens).
</Callout>
...
See [API keys & access](/en/docs/general-concepts/api-keys-and-access) for the full key model.
