...
Prefer to paste a token — or on a headless machine or CI? That flow is still supported: pass `--no-browser` to paste a token you minted under **Organization Settings → CLI tokens**, or `--token <koda_…>` to sign in non-interactively. See [API keys & access](/id/docs/general-concepts/api-keys-and-access#cli-tokens).
</Callout>
