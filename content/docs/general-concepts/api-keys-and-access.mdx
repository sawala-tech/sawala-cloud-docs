...
Every key is tied to the **project** (or projects) you choose when you mint it. A key can only read and write data in the projects it's scoped to — it can never reach another project's content. This is how [project isolation](/id/docs/general-concepts/organizations-and-projects) is enforced at the API layer.
...
See each product's API reference for the full endpoint list — for example, the [Kontena API reference](/id/docs/kontena/reference/api).
...
<Callout type="info">
The dashboard, the CLIs, and the MCP servers authenticate differently under the hood — see [CLI & SDKs](/id/docs/general-concepts/cli-and-sdk). As an app developer using an SDK or the REST API directly, you only need an **API key** as described here.
</Callout>

Next: put it all together in [Getting started](/id/docs/general-concepts/getting-started).
