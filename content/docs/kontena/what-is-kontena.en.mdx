...
- You want media (images, video, documents) managed alongside text, backed by [Berkasna](/en/docs/berkasna/what-is-berkasna).
...
- **Public API key** — a read-only token (`pk_live_…`) that is safe to use from a browser. See [API keys and access](/en/docs/general-concepts/api-keys-and-access).

Schemas live inside a project, and projects belong to an organization. See [organizations and projects](/en/docs/general-concepts/organizations-and-projects) for how that hierarchy works.
...
The `media` field type is wired directly to [Berkasna](/en/docs/berkasna/what-is-berkasna), so images and documents you reference in an entry are managed and served by Sawala Cloud's media product.
